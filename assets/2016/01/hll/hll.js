...
function HyperLogLog(std_error) {
    ...
    function count(hash) {
        function count_float() {
            var c = estimators.reduce(function (acc, estimate) {
                return acc + 1 / Math.pow(2, estimate);
            }, 0);
            var E = alpha_m * m * m / c;
            if (E <= min_estimate) {
                var V = estimators.filter(function (estimate) {
                    return estimate === 0;
                }).length > 0;
