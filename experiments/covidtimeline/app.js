...
const size = () => {
  ...
  return {
    width: sizeRatio.width * window.innerWidth - left - right,
    height: sizeRatio.height * window.innerHeight - top - bottom,
  };
};

const originalKeys = ["T", "B", "Noe", "Ooe", "W", "S", "V", "K", "St"];
...
const Main = ({ keys, x, y, width, height }) => {
  ...
  const maximaIndices = React.useMemo(
    () =>
      keys
        .map((key, keyIdx) => {
          let max = 0;
  ...
  const sortedKeys = React.useMemo(
    () => maximaIndices.map(([idx]) => originalKeys[idx]),
    [maximaIndices],
  );

  const lineHeight = height / sortedKeys.length;
...
const Svg = () => {
  ...
  return e(
    ...
    e(
      ...
      width > 0 &&
        height > 0 &&
        e(Main, { x: left, y: top, width, height, keys: originalKeys }),
    ),
  );
};
